...
const SUBJECT_VALUES = ["demo", "pricing", "support", "other"] as const;
  ...
  if (submitted) {
    return (
      <div className="flex flex-col items-center justify-center rounded-2xl border border-primary/20 bg-primary/5 p-12 text-center">
        ...
        <h3 className="mt-2 text-lg font-semibold text-charcoal">{t("successTitle")}</h3>
        <p className="mt-2 text-sm text-charcoal-light">{t("successBody")}</p>
        <button
          type="button"
    ...
    <section id="contact" className="py-24 lg:py-32 bg-cream">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          <p className="mt-4 text-lg text-charcoal-light">{t("intro")}</p>
        </div>
        <div className="grid md:grid-cols-3 gap-10 items-start">
          <div className="md:col-span-1 bg-cream rounded-2xl p-8 border border-stone-200 hover:border-primary/30 transition-all flex flex-col gap-8">
            <div>
              <h3 className="text-lg font-semibold text-charcoal mb-4">{t("infoTitle")}</h3>
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  <div className="text-xs text-stone-500 uppercase tracking-wider">{t("emailLabel")}</div>
                  <a
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="text-xs text-stone-500 uppercase tracking-wider">{t("phoneLabel")}</div>
          ...
          <form onSubmit={handleSubmit} className="md:col-span-2 space-y-6 bg-white rounded-2xl shadow-lg p-8 border border-stone-100">
            {error && (
              <div className="flex items-start gap-3 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
                <svg className="mt-0.5 h-4 w-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.054 0 1.918-.816 1.995-1.85l.007-.15V6c0-1.054-.816-1.918-1.85-1.995L19 4H5c-1.054 0-1.918.816-1.995 1.85L3 6v12c0 1.054.816 1.918 1.85 1.995L5 20z" />
                </svg>
                {error}
              </div>
            )}
            <div className="grid gap-6 md:grid-cols-2">
              <div>
                ...
                <input
                  ...
                  type="text"
                  required
                  className="mt-1 block w-full rounded-md border border-stone-200 bg-cream px-3 py-2 text-charcoal placeholder-stone-500 shadow-sm focus:border-primary focus:ring-primary"
            ...
            <div>
              ...
              <select
                ...
                name="subject"
                required
                className="mt-1 block w-full rounded-md border border-stone-200 bg-cream px-3 py-2 text-charcoal shadow-sm focus:border-primary focus:ring-primary"
              >
            ...
            >
              {loading ? (
                ...
                <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2 12l9 9 11-11" />
                </svg>
              )}
