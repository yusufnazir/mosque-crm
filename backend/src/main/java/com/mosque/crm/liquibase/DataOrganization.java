...
public class DataOrganization extends CustomDataTaskChange {
	...
	public void handleUpdate() throws DatabaseException, SQLException {
		String query = "select id from organizations where id=?";
		...
		if (exists) {
			String update = "update organizations set name=?, short_name=?, address=?, city=?, country=?, postal_code=?, phone=?, email=?, website=?, active=? where id=?";
			...
			String insert = "insert into organizations(id, name, short_name, address, city, country, postal_code, phone, email, website, active) values(?,?,?,?,?,?,?,?,?,?,?)";
